...
AABSection::AABSection()
{
	...
	{
		ABLOG(Error, TEXT("Faile to load staticmesh asset. : %s"), *AssetPath);
	}
...
void AABSection::SetState(ESectionState NewState)
{
	...
	case AABSection::ESectionState::BATTLE:
		...
		GetWorld()->GetTimerManager().SetTimer(SpawnNPCTimerHandles, FTimerDelegate::CreateUObject(this, &AABSection::OnNPCSpawn), EnemySpawnTime, false);
		GetWorld()->GetTimerManager().SetTimer(SpawnItemBoxTimerHandles, FTimerDelegate::CreateLambda([this]()->void{
...
void AABSection::OnGateTriggerBeginOverlap(UPrimitiveComponent* OverlappedComponent, AActor* OtherActor, UPrimitiveComponent* OtherComp, int32 OtherBodyIndex, bool bFromSweep, const FHitResult& SweepResult)
{
	...
	FName ComponetTag = OverlappedComponent->ComponentTags[0];
	FName SocketName = FName(*ComponetTag.ToString().Left(2));
...
{
	GetWorld()->GetTimerManager().ClearTimer(SpawnNPCTimerHandles);
    AABCharacter* KeyNPC =GetWorld()->SpawnActor<AABCharacter>(GetActorLocation() + FVector::UpVector * 88.0f, FRotator::ZeroRotator);
	if (nullptr != KeyNPC)
	{
